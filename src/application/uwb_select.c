...
//return NULL otherwise
 static timing_node *select(struct TimedSelector *this)
 { 
...
static bool add_node(struct TimedSelector *this, timing_node *node) //TODO handle unsuccessful add
{    
 ...
 static bool remove_node(struct TimedSelector *this, timing_node *node){ //TODO handle unsuccessful delete
     return llist_delete_node(this->list, node, true, NULL); //TODO fix return type
 }

static struct TimedSelector new(){
    return (struct TimedSelector){
...
const struct TimedSelectorClass TimedSelector={.new=&new};